...
def postprocess_bayer(rawpath, img4c):
    ...
    out = raw.postprocess(
        use_camera_wb=True, half_size=True, no_auto_bright=True, output_bps=8, bright=1, user_black=None, user_sat=None
    )
    return out
                ...
                if self.data_name is not None:
                    ...
                    if frame_id is not None:
                        ...
                        Image.fromarray(output.astype(np.uint8)).save(
                            join(savedir, name, self.opt.name, "{}_{:.2f}.png".format(frame_id, res["PSNR"]))
                        )

                        if not os.path.exists(
                            join(savedir, name, "input", "{}_{:.2f}.png".format(frame_id, res_in["PSNR"]))
                        ):
                            Image.fromarray(input.astype(np.uint8)).save(
                                join(savedir, name, "input", "{}_{:.2f}.png".format(frame_id, res_in["PSNR"]))
                            )
                        ...
                        if not os.path.exists(join(savedir, name, "label", "{}.png".format(frame_id))):
                            Image.fromarray(target.astype(np.uint8)).save(
                                join(savedir, name, "label", "{}.png".format(frame_id))
                            )
                    else:
                        if suffix is not None:
                            Image.fromarray(output.astype(np.uint8)).save(
                                join(savedir, name, "{}_{:.1f}_{}.png".format(self.opt.name, res["PSNR"], suffix))
                            )
                            ...
                            Image.fromarray(input.astype(np.uint8)).save(
                                join(savedir, name, "m_input_{}.png".format(suffix))
                            )
                            ...
                            Image.fromarray(output.astype(np.uint8)).save(
                                join(savedir, name, "{}_{:.2f}.png".format(self.opt.name, res["PSNR"]))
                            )
                            # Image.fromarray(output.astype(np.uint8)).save(join(savedir, name, '{}.png'.format(self.opt.name)))
                            Image.fromarray(input.astype(np.uint8)).save(
                                join(savedir, name, "m_input_{:.2f}.png".format(res_in["PSNR"]))
                            )
    ...
    def test(self, data, savedir=None, video_mode=False):
        ...
        with torch.no_grad():
            ...
            if self.rawpath:
                ...
                if not video_mode:
                    Image.fromarray(output.astype(np.uint8)).save(
                        join(savedir, name, "{}.jpg".format(self.opt.name)), optimize=True, quality=90
                    )
                    Image.fromarray(input.astype(np.uint8)).save(
                        join(savedir, name, "m_input.jpg"), optimize=True, quality=90
                    )
                else:
                    Image.fromarray(output.astype(np.uint8)).save(
                        join(savedir, self.opt.name, "{}.jpg".format(name)), optimize=True, quality=90
                    )
...
class ELDModel(ELDModelBase):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.optimizer_G = torch.optim.Adam(
                self.netG.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999), weight_decay=opt.wd
            )
    ...
    def backward_G(self):
        ...
        self.loss_pixel = self.loss_dic["pixel"].get_loss(self.output, self.target)
    ...
    def forward_chop(self, x, base=16):
        ...
        output[:, :, h_half:h, w_half:w] = outputs[3][
