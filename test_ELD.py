...
for i, img_ids in enumerate(img_ids_set):
    ...
    eval_datasets = [
        datasets.ELDEvalDataset(databasedir, camera_suffix, scenes=scenes, img_ids=img_ids)
        for camera_suffix in zip(cameras, suffixes)
    ]

    eval_dataloaders = [
        torch.utils.data.DataLoader(
            eval_dataset, batch_size=1, shuffle=False, num_workers=opt.nThreads, pin_memory=True
        )
