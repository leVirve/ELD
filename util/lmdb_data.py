...
def Data2Volume(data, ksizes, strides):
    ...
    V = np.zeros([int(TotalPatNum)] + ksizes)
...
def create_sony_dataset_paired(num_samples=None):  # paired real data
    ...
    create_lmdb_train(
        [join(sourcedir, "short", fn[0]) for fn in fns],
        join(destdir, "SID_Sony_input_Raw"),
        ksize=(4, 512, 512),
        stride=(4, 512, 512),
        ratios=ratios,
        srgb=False,
    ...
    create_lmdb_train(
        [join(sourcedir, "long", fn[1]) for fn in fns],
        join(destdir, "SID_Sony_target_Raw"),
        ksize=(4, 512, 512),
        stride=(4, 512, 512),
        srgb=False,
    )
...
def create_sony_dataset_SRGB(num_samples=None):  # synthetic data
    ...
    create_lmdb_train(
        fns, join(destdir, "SID_Sony_SRGB_CRF"), ksize=(3, 512, 512), stride=(3, 512, 512), srgb=True, CRF=CRF
    )
...
def create_sony_dataset_SRGB_paired(num_samples=None):  # paired real data
    ...
    create_lmdb_train(
        [join(sourcedir, "short", fn[0]) for fn in fns],
        join(destdir, "SID_Sony_input_SRGB_CRF"),
        ksize=(3, 512, 512),
        stride=(3, 512, 512),
        ratios=ratios,
        srgb=True,
    ...
    create_lmdb_train(
        [join(sourcedir, "long", fn[1]) for fn in fns],
        join(destdir, "SID_Sony_target_SRGB_CRF"),
        ksize=(3, 512, 512),
        stride=(3, 512, 512),
        srgb=True,
        CRF=CRF,
