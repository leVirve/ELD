...
def get_model_list(dirname, key, epoch=None):
    ...
    gen_models = [
        ...
        if os.path.isfile(os.path.join(dirname, f)) and key in f and ".pt" in f and "latest" not in f
    ]
    if gen_models is None:
        return None

    epoch_index = [
        int(os.path.basename(model_name).split("_")[-2])
        for model_name in gen_models
        if os.path.basename(model_name).split("_")[-2].isdigit()
    ]
