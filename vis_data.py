...
test_fns_list = [
    [
        fn
        for fn in test_fns
        if min(int(read_expo_ratio(fn[1]) / read_expo_ratio(fn[0])), 300) == ratio
        and int(fn[0][:5]) not in [10192, 10199, 10203]
    ]
    for ratio in expo_ratio
]
...
target_data = LMDBDataset(join(traindir, "SID_Sony_Raw.db"), size=opt.max_dataset_size, repeat=repeat)
...
camera = "NikonD850"
input_data = LMDBDataset(join(traindir, f"SID_Sony_syn_Raw_{camera}.db"), size=opt.max_dataset_size, repeat=repeat)


train_dataset = datasets.ELDTrainDataset(
    target_dataset=target_data, input_datasets=[input_data], size=10, augment=False
)
