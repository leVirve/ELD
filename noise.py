...
class RawPacker:
    ...
    def pack_raw_bayer(self, cfa_img):
        ...
        out = np.stack(
            (cfa_img[0:H:2, 0:W:2], cfa_img[0:H:2, 1:W:2], cfa_img[1:H:2, 1:W:2], cfa_img[1:H:2, 0:W:2]), axis=0  # RGBG
        ).astype(np.float32)
        return out
...
class NoiseModel(NoiseModelBase):
    ...
    def _sample_params(self):
        ...
        log_g_scale = (
            np.random.standard_normal() * camera_params["g_scale"]["sigma"] * 1
            + camera_params["g_scale"]["slope"] * log_K
            + camera_params["g_scale"]["bias"]
        )
