...
if opt.stage_out == "srgb":
    ...
    target_data = lmdb_dataset.LMDBDataset(join(traindir, "SID_Sony_Raw.db"), size=opt.max_dataset_size, repeat=repeat)

if opt.stage_in == "srgb":
    input_data = datasets.ISPDataset(
        lmdb_dataset.LMDBDataset(join(traindir, "SID_Sony_Raw.db")), noise_maker=noise_model, CRF=CRF
    )
else:
    ...
    input_data = lmdb_dataset.LMDBDataset(
        join(traindir, f"SID_Sony_syn_Raw_{camera}.db"), size=opt.max_dataset_size, repeat=repeat
    )
