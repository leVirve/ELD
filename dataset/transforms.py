...
import torch
from PIL import Image, ImageEnhance, ImageOps, PILLOW_VERSION
...
def get_transform(opt):
    ...
    if opt.resize_or_crop == "resize_and_crop":
        transform_list.append(
            transforms.RandomChoice([transforms.Resize([osize, osize], Image.BICUBIC) for osize in osizes])
        )
        transform_list.append(transforms.RandomCrop(fineSize))
    elif opt.resize_or_crop == "crop":
        transform_list.append(transforms.RandomCrop(fineSize))
    elif opt.resize_or_crop == "scale_width":
        transform_list.append(transforms.Lambda(lambda img: __scale_width(img, fineSize)))
...
to_norm_tensor = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
...
class ReflectionSythesis_1(object):
    ...
    def __init__(self, kernel_sizes=None, low_sigma=2, high_sigma=5, low_gamma=1.3, high_gamma=1.3):
        ...
        print(
            "[i] reflection sythesis model: {}".format(
                {
